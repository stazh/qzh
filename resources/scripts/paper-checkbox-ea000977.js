import { d as directive, L as LitElement, N as NodePart, A as AttributePart } from './pb-mixin-ae9e2885.js';
...
function dispatchNotifyEvent(inst, eventName, value, path) {
  ...
  wrap$1(inst).dispatchEvent(new CustomEvent(eventName, {
    detail
  }));
}
/**
...
const PropertyEffects = dedupingMixin(superClass => {
  ...
  class PropertyEffects extends propertyEffectsBase {
    ...
    _setPendingPropertyOrPath(path, value, shouldNotify, isPathNotification) {
      if (isPathNotification || root(Array.isArray(path) ? path[0] : path) !== path) {
        ...
        if (!isPathNotification) {
          ...
          path = set(this, path, value); // Use property-accessor's simpler dirty check
    ...
    splice(path, start, deleteCount, ...items) {
      ...
      let array = get(this, path, info); // Normalize fancy native splice handling of crazy start values
...
function _fire(target, type, detail) {
  ...
  ev.detail = detail;
  wrap$1(target).dispatchEvent(ev); // forward `preventDefault` in a clean way
...
let FlattenedNodesObserver = class {
  ...
  static getFlattenedNodes(node) {
    ...
    if (isSlot(node)) {
      node = node; // eslint-disable-line no-self-assign
  ...
  connect() {
    if (isSlot(this._target)) {
      ...
      this._listenSlots(wrap$1(this._target).children);
  ...
  disconnect() {
    if (isSlot(this._target)) {
      ...
      this._unlistenSlots(wrap$1(this._target).children);
...
class DomIfBase extends PolymerElement {
  ...
  __ensureTemplate() {
    if (!this.__template) {
      ...
      // itself, which has been given a `_templateInfo` property
      const thisAsTemplate = this;
      let template = thisAsTemplate._templateInfo ? thisAsTemplate : wrap$1(thisAsTemplate).querySelector('template');
...
const partCache = new Map(); // The currently used i18next translation function
...
function initTranslation(translate) {
...
function updatePart(part, cb) {
  // Grab the new value
  const newValue = cb(); // Only set the value if it has changed
...
const translate$1 = (key, value) => langChanged(() => get$1(key, value));
...
class PbI18n extends LitElement {
  static get properties() {
  ...
  _translate() {
    const transl = get$1(this.key, this.options);
...
const IronFitBehavior = {
  ...
  position: function () {
    ...
    this.sizingTarget.style.maxHeight = maxHeight + 'px'; // Remove the offset caused by any stacking context.
    ...
    if (this.expandSizingTargetForScrollbars) {
      ...
      if (sizingTargetScrollbarHeight > 0) {
        ...
        this.sizingTarget.style.maxHeight = `${newMaxHeight}px`; // Measure the element's real change in height. This may not equal
        // `sizingTargetScrollbarHeight` if the overflow amount is less than the
        // scrollbar size.

        const offsetHeight = this.sizingTarget.offsetHeight;
        const addedHeight = offsetHeight - positionedOffsetHeight; // Adjust the top position if the alignment requires it.
      ...
      if (sizingTargetScrollbarWidth > 0) {
        const maxWidthBugOffset = getVerticalScrollbarMaxWidthBugOffset(); // Expand `maxWidth` by `sizingTargetScrollbarWidth` up to the overall
        ...
        const addedWidth = offsetWidth - positionedOffsetWidth; // Adjust the left position if the alignment requires it.
...
class IronOverlayManagerClass {
  ...
  _onCaptureClick(event) {
    ...
    while ((overlay = this._overlays[i]) && this._overlayInPath(path) !== overlay) {
...
function _getScrollableNodes(nodes) {
  ...
  for (var i = 0; i <= lockingIndex; i++) {
    ...
    var node = nodes[i]; // Check inline style before checking computed style.
...
function _getScrollInfo(event) {
  ...
  if ('deltaX' in event) ; // Safari has scroll info in `wheelDeltaX/Y`.
...
`,
  ...
  _isPrintable: function (event) {
    ...
    var anyNonPrintable = event.keyCode == 8 || event.keyCode == 9 || event.keyCode == 13 || event.keyCode == 27; // escape
    // For these keys, make sure it's a browser keycode and not an ASCII code.

    var mozNonPrintable = event.keyCode == 19 || event.keyCode == 20 || event.keyCode == 45 || event.keyCode == 46 || event.keyCode == 144 || event.keyCode == 145 || event.keyCode > 32 && event.keyCode < 41 || event.keyCode > 111 && event.keyCode < 124; // fn keys
...
const IronMenuBehaviorImpl = {
  ...
  _onFocus: function (event) {
    ...
    var rootTarget = dom(event).rootTarget;
...
export { DomModule as A, Base as B, idlePeriod as C, Debouncer as D, timeOut as E, FlattenedNodesObserver as F, GestureEventListeners as G, mixinBehaviors as H, IronControlState as I, NeonAnimationRunnerBehavior as N, Polymer as P, Templatizer as T, PaperButtonBehavior as a, IronButtonState as b, PaperRippleBehavior as c, dom as d, IronMenuBehavior as e, IronResizableBehavior as f, get$1 as g, html as h, initTranslation as i, IronMenuBehaviorImpl as j, IronSelectableBehavior as k, afterNextRender as l, animationFrame as m, enqueueDebouncer as n, PolymerElement as o, addListener as p, NeonAnimatableBehavior as q, resetMouseCanceller as r, NeonAnimationBehavior as s, translate$1 as t, IronValidatableBehavior as u, PaperInputBehavior as v, IronFormElementBehavior as w, resolveUrl as x, flush as y, PaperInkyFocusBehavior as z };
//# sourceMappingURL=paper-checkbox-ea000977.js.map
