            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='context') then
                            let $params := 
                                map {
                                    "persons": let $register := doc("/db/apps/rqzh-data/person/person.xml") let $entries:=     for $p in root(.)//persName[@ref]   group by $k := $p/@ref     return     id($p[1]/@ref , $register)  return  for $p in $entries   order by $p/persName[@type="full_sorted"] ascending   return    $p,
