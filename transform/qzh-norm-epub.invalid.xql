(:~
    ...
    ODD: /db/apps/qzh/resources/odd/qzh-norm.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/qzh-norm/epub";
                        ...
                        if (@ref) then
                            ...
                            let $params := 
                                map {
                                    ...
                                    "value": let $id := replace(@ref, ' ', '-') let $target:= id($id, doc("/db/apps/qzh-data/organization/organization.xml")) let $letter := substring($id, 1, 1) => upper-case() return     if ($target) then ( <a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$target/orgName/string()}</a>, if ($target/@type) then( <span> ({$target/@type/string()})</span>) else () ) else (<a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$id}</a>)
                                }
                                ...
                                if (@ref) then
                                    ...
                                    let $params := 
                                        map {
                                            ...
                                            "value": let $id := replace(@ref, ' ', '-') let $target:= id($id, doc("/db/apps/qzh-data/person/person.xml")) let $letter := substring($id, 1, 1) => upper-case() return     if ($target) then ( <a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$target/persName/string()}</a>, if ($target/note) then( <span> ({$target/note/string()} )</span>) else () ) else (<a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$id}</a>)
                                        }
