...
module namespace model="http://www.tei-c.org/pm/models/qzh/latex";
...
import module namespace ext-latex="http://www.tei-c.org/tei-simple/xquery/functions/ssrq-latex" at "xmldb:exist:///db/apps/qzh/modules/ext-latex.xql";
            ...
            return
                    ...
                    case element(choice) return
                        if (sic and corr) then
                            ext-latex:alternote($config, ., ("tei-choice1", "text-critical", css:map-rend-to-class(.)), sic, (), 'text-critical', corr, map {"prefix": (ec:label('corrected'), ec:colon())})
                        else
                            if (abbr and expan) then
                                ext-latex:alternate($config, ., ("tei-choice2", "text-critical", css:map-rend-to-class(.)), ., abbr[1], expan[1])
                            else
                                if (orig and reg) then
                                    ext-latex:alternate($config, ., ("tei-choice3", "text-critical", css:map-rend-to-class(.)), ., orig[1], reg[1])
                                else
                                    $config?apply($config, ./node())
                    ...
                    case element(seg) return
                        latex:inline($config, ., ("tei-seg", css:map-rend-to-class(.)), .)
                    ...
                    case element(lb) return
                        if (ancestor::back//orig) then
                            ...
                            if (preceding-sibling::node()[1][self::pb]) then
                                latex:omit($config, ., ("tei-lb2", css:map-rend-to-class(.)), .)
                            else
                                if (. is ancestor::div/node()[1]) then
                                    latex:omit($config, ., ("tei-lb3", css:map-rend-to-class(.)), .)
                                else
                                    if (. is ancestor::p/node()[1]) then
                                        latex:omit($config, ., ("tei-lb4", css:map-rend-to-class(.)), .)
                                    else
                                        if (@break='no') then
                                            (
                                                latex:inline($config, ., ("tei-lb5", css:map-rend-to-class(.)), .),
                                                latex:break($config, ., ("tei-lb6", css:map-rend-to-class(.)), ., 'page', @n)
                                            )

                                        else
                                            latex:break($config, ., ("tei-lb7", css:map-rend-to-class(.)), ., 'line', @n)
                    ...
                    case element(head) return
                        if (@resp) then
                            ...
                            if (@type='title') then
                                latex:heading($config, ., ("tei-head2", css:map-rend-to-class(.)), ., 1)
                            else
                                if (@type='subtitle') then
                                    latex:heading($config, ., ("tei-head3", css:map-rend-to-class(.)), ., 2)
                                else
                                    if ($parameters?header='short') then
                                        latex:heading($config, ., ("tei-head5", css:map-rend-to-class(.)), replace(string-join(.//text() except .//ref//text()), '^(.*?)[^\w]*$', '$1'), ())
                                    else
                                        if (parent::figure) then
                                            ...
                                            if (parent::table) then
                                                ...
                                                if (parent::lg) then
                                                    ...
                                                    if (parent::list) then
                                                        latex:block($config, ., ("tei-head9", css:map-rend-to-class(.)), .)
                                                    else
                                                        if (ancestor::app) then
                                                            (: Heading in app (don't format) :)
                                                            latex:inline($config, ., ("tei-head10", css:map-rend-to-class(.)), .)
                                                        else
                                                            latex:heading($config, ., css:get-rendition(., ("tei-head4", css:map-rend-to-class(.))), ., 2)
                    ...
                    case element(orgName) return
                        if (@ref) then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-orgName", "semantic", "organization", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(placeName) return
                        if (@ref) then
                            ...
                            return
                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-placeName", "semantic", "place", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
