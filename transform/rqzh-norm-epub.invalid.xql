...
module namespace model="http://www.tei-c.org/pm/models/rqzh-norm/epub";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(handShift) return
                        epub:note($config, ., ("tei-handShift", css:map-rend-to-class(.)), (ec:label('handswitch'), if (@scribe) then (ec:colon(), ec:scribe(@scribe)) else ()), 'footnote', ())
                        ...
                        if ($parameters?header='context') then
                            let $params := 
                                map {
                                    "content": .,
                                    "ref": @xml:id,
                                    "value": (<a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={@xml:id}" target="_blank">{orgName/string()}</a>, if (@type) then( <span> ({@type/string()})</span>) else ())
                                }

                                                        let $content := 
                                model:template-org($config, ., $params)
                            return
                                                        html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-org", css:map-rend-to-class(.)), $content)
                        else
                    ...
                    case element(teiHeader) return
                        if ($parameters?header='context') then
                            ...
                            if ($parameters?mode='footer-prev') then
                                ...
                                return
                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader2", "footer", "button-prev", css:map-rend-to-class(.)), $content)
                            else
                                if ($parameters?mode='footer-next') then
                                    ...
                                    return
                                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader3", "footer", "button-next", css:map-rend-to-class(.)), $content)
                                else
                                    if ($parameters?view='metadata') then
                                        (
                                            if (not(ends-with(util:document-name(.//sourceDesc), 'Introduction.xml'))) then
                                                epub:block($config, ., ("tei-teiHeader4", css:map-rend-to-class(.)), fileDesc)
                                            else
                                                (),
                                            epub:block($config, ., ("tei-teiHeader5", css:map-rend-to-class(.)), .//msDesc/head),
                                            ...
                                            if (exists(.//msContents/summary/node())) then
                                                ...
                                                return
                                                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader7", css:map-rend-to-class(.)), $content)
                                            else
                                                (),
                                            if (root($parameters?root)/TEI[not(@type) or @type != 'introduction']) then
                                                ...
                                                return
                                                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader8", css:map-rend-to-class(.)), $content)
                                            else
                                                (),
                                            if (root($parameters?root)/TEI[not(@type) or @type != 'introduction'] and ec:existsAdditionalSource(.//fileDesc/seriesStmt/idno/text()))) then
                                                ...
                                                return
                                                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-teiHeader9", css:map-rend-to-class(.)), $content)
                                            else
                                                ()
                                        )
                                        ...
                                        if ($parameters?header='short') then
                                            epub:block($config, ., ("tei-teiHeader11", css:map-rend-to-class(.)), .)
                                        else
                                            $config?apply($config, ./node())
                    ...
                    case element(supplied) return
                        if (parent::choice) then
                            ...
                            if (@source and @reason) then
                                (: No function found for behavior: alternote :)
                                $config?apply($config, ./node())
                            else
                                if (@source) then
                                    (: No function found for behavior: alternote :)
                    ...
                    case element(idno) return
                        if (parent::altIdentifier) then
                            ...
                            if (@source) then
                                html:link($config, ., ("tei-idno2", css:map-rend-to-class(.)), (ec:label('signature'), ec:colon(), .), (), '_new', map {"link": @source})
                            else
                    ...
                    case element(signed) return
                        epub:block($config, ., ("tei-signed", css:map-rend-to-class(.)), ('[', ec:label('signed'), ec:punct(':', false()), '] ', .))
                    ...
                    case element(TEI) return
                        if ($parameters?header='context') then
                            ...
                            if ($parameters?view='metadata') then
                                (
                                    epub:block($config, ., ("tei-TEI2", "document-heading", css:map-rend-to-class(.)), teiHeader),
                                    epub:block($config, ., ("tei-TEI3", css:map-rend-to-class(.)), text//back)
                                )
                        ...
                        if (sic/del and corr) then
                            ...
                            if (sic and corr) then
                                ...
                                if (abbr and expan) then
                                    epub:alternate($config, ., ("tei-choice6", "text-critical", css:map-rend-to-class(.)), ., expan[1], (ec:label('abbr-nonexpanded'), ec:colon(), abbr[1]))
                                else
                                    if (orig and reg) then
                                        epub:alternate($config, ., ("tei-choice7", "text-critical", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(hi) return
                        if (@rend='sup') then
                            ...
                            if (@rend!='sup' and @hand) then
                                ...
                                if (@rend!='sup') then
                                    epub:alternate($config, ., ("tei-hi3", "text-critical", css:map-rend-to-class(.)), ., ., ec:translate(@rend))
                                else
                                    $config?apply($config, ./node())
                    case element(note) return
                        if (@place) then
                            ...
                            epub:note($config, ., ("tei-note2", css:map-rend-to-class(.)), ., (), ())
                    ...
                    case element(addSpan) return
                        if (parent::subst) then
                            ...
                            epub:note($config, ., ("tei-addSpan2", css:map-rend-to-class(.)), (ec:label('add'), ' ', string-join((ec:translate(@place, 0, ()), ec:translate(@hand, 0, 'lowercase'), ec:translate(@rend, 0, 'lowercase')), ' '), if (@scribe) then (' ', ec:label('byScribe', false()), ' ', ec:scribe(@scribe)) else ()), "footnote", ())
                    ...
                    case element(back) return
                        if ($parameters?view='metadata' and exists(./div/p/node())) then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-back1", css:map-rend-to-class(.)), $content)
                        else
                            if (div/@n) then
                                html:list($config, ., ("tei-back2", css:map-rend-to-class(.)), ., 'ordered')
                            else
                                epub:block($config, ., ("tei-back3", css:map-rend-to-class(.)), .)
                    ...
                    case element(del) return
                        if (ancestor::back//orig) then
                            ...
                            if (add/@type='catchword') then
                                epub:note($config, ., ("tei-del2", css:map-rend-to-class(.)), ., (), ())
                            else
                                if (@hand='later hand' and not(parent::subst)) then
                                    ...
                                    if (add) then
                                        epub:note($config, ., ("tei-del4", css:map-rend-to-class(.)), ., (), ())
                                    else
                                        if (parent::subst) then
                                            ...
                                            if (gap) then
                                                ...
                                                if (unclear and @rend) then
                                                    epub:note($config, ., ("tei-del7", css:map-rend-to-class(.)), ., "footnote", ())
                                                else
                                                    if (unclear) then
                                                        epub:note($config, ., ("tei-del8", css:map-rend-to-class(.)), ., "footnote", ())
                                                    else
                                                        if (@rend) then
                                                            (: Show footnote only :)
                                                            epub:note($config, ., ("tei-del9", css:map-rend-to-class(.)), ., (), ())
                                                        else
                                                            if (@hand) then
                                                                (: Show footnote only :)
                                                                epub:note($config, ., ("tei-del10", css:map-rend-to-class(.)), ., (), ())
                                                            else
                                                                (: Show footnote only :)
                                                                epub:note($config, ., ("tei-del11", css:map-rend-to-class(.)), ., (), ())
                    ...
                    case element(ab) return
                        if (@place) then
                            epub:block($config, ., ("tei-ab1", "ab", css:map-rend-to-class(.)), ('[', string-join((ec:translate(@type, 0, 'lowercase'),ec:translate(@place, 0, 'lowercase'),ec:translate(@hand, 0, 'lowercase')), ' '), if (@scribe) then (' ', ec:label('byScribe', false()), ' ', ec:scribe(@scribe)) else (), ec:punct(':', false()), '] ', .))
                        else
                    ...
                    case element(damageSpan) return
                        if (@agent) then
                            epub:note($config, ., ("tei-damageSpan1", css:map-rend-to-class(.)), (ec:label('damage'), ' ', ec:label('through', false()), ' ', ec:translate(@agent, 0, ())), "footnote", ())
                        else
                    ...
                    case element(foreign) return
                        if ((session:get-attribute('ssrq.lang'), 'de')[1]='fr') then
                            epub:alternate($config, ., ("tei-foreign1", css:map-rend-to-class(.)), ., ., (ec:label('lang-switch'), ec:colon(), ' ', ec:label(@xml:lang, false())))
                        else
                            epub:alternate($config, ., ("tei-foreign2", css:map-rend-to-class(.)), ., ., (ec:label('lang-switch'), ec:colon(), ' ', ec:label(@xml:lang, false())))
                    case element(fileDesc) return
                        if ($parameters?header='short' and ancestor::TEI[@type='introduction']) then
                            (
                                epub:block($config, ., ("tei-fileDesc1", "header-short", css:map-rend-to-class(.)), ec:format-id(seriesStmt/idno)),
                                html:link($config, ., ("tei-fileDesc2", "header-short", "title", css:map-rend-to-class(.)),                              let $subtype := ancestor::TEI/@subtype                              return switch ($subtype)                                 case 'a' return 'Reihenvorwort'                                 case 'b' return 'Vorwort'                                 case 'c' return 'Einleitung'                                 case 'd' return 'Quellenverzeichnis'                                 default return 'Introduction'                                 , (), (), map {"link": $parameters?doc})
                            )

                        else
                            if ($parameters?header='short') then
                                (
                                    epub:block($config, ., ("tei-fileDesc3", "header-short", css:map-rend-to-class(.)), ec:format-id(seriesStmt/idno)),
                                    html:link($config, ., ("tei-fileDesc4", "header-short", css:map-rend-to-class(.)), sourceDesc/msDesc/head, (), (), map {"link": $parameters?doc}),
                                    epub:block($config, ., ("tei-fileDesc5", "header-short", css:map-rend-to-class(.)), if (exists(sourceDesc/msDesc/msContents/msItem/filiation[@type='original'][origDate])) then                                      ec:print-date(sourceDesc/msDesc/msContents/msItem/filiation[@type='original']/origDate)                                     else if (exists(sourceDesc/msDesc/history/origin/origDate)) then                                     ec:print-date(sourceDesc/msDesc/history/origin/origDate)                                    else                                       ()),
                                    epub:block($config, ., ("tei-fileDesc6", "header-short", css:map-rend-to-class(.)), editionStmt),
                                    epub:block($config, ., ("tei-fileDesc7", "header-short", css:map-rend-to-class(.)), publicationStmt),
                                    epub:block($config, ., ("tei-fileDesc8", "header-short", css:map-rend-to-class(.)), titleStmt)
                                )
                                ...
                                if ($parameters?view='volumes') then
                                    ...
                                    return
                                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-fileDesc9", css:map-rend-to-class(.)), $content)
                                else
                                    if ($parameters?view='metadata' and ends-with(util:document-name($get(.)), 'Einleitung.xml')) then
                                        ...
                                        return
                                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-fileDesc10", css:map-rend-to-class(.)), $content)
                                    else
                                        if ($parameters?view='metadata') then
                                            ...
                                            return
                                                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-fileDesc11", css:map-rend-to-class(.)), $content)
                                        else
                                            $config?apply($config, ./node())
                    case element(delSpan) return
                        if (not(@rend)) then
                            epub:note($config, ., ("tei-delSpan1", css:map-rend-to-class(.)), ec:label('del'), (), ())
                        else
                            epub:note($config, ., ("tei-delSpan2", css:map-rend-to-class(.)), (ec:translate(@rend, 0, 'uppercase'), if (@hand) then (' ', ec:translate(@hand, 0, 'lowercase')) else ()), (), ())
                    case element(seg) return
                        if (@n and not(p)) then
                            epub:block($config, ., ("tei-seg1", css:map-rend-to-class(.)), (text{'['}, @n, text{'] '}, .))
                        else
                            if (. is (ancestor::quote[1]/seg[1])) then
                                (: Force « on same line, cf. quote :)
                                html:inline($config, ., ("tei-seg2", css:map-rend-to-class(.)), .)
                            else
                                if (. is (ancestor::quote[1]/seg[last()])) then
                                    (: Force » on same line, cf. quote :)
                                    html:inline($config, ., ("tei-seg3", css:map-rend-to-class(.)), .)
                                else
                                    if (. is (ancestor::quote[1]/seg[last()-1])) then
                                        (: Add missing margin :)
                                        epub:block($config, ., ("tei-seg4", css:map-rend-to-class(.)), .)
                                    else
                                        epub:block($config, ., ("tei-seg5", css:map-rend-to-class(.)), .)
                    ...
                    case element(place) return
                        if ($parameters?header='context') then
                            ...
                            return
                                                        html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-place", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(abbr) return
                        if (parent::choice) then
                            ...
                            if (unclear) then
                                epub:alternate($config, ., ("tei-abbr2", css:map-rend-to-class(.)), ., ., (ec:label('abbr-unclear'), ec:colon(), ec:abbr(.)))
                            else
                                epub:alternate($config, ., ("tei-abbr3", css:map-rend-to-class(.)), ., ., (ec:label('abbr'), ec:colon(), ec:abbr(.)))
                        ...
                        if (@ref) then
                            ...
                            let $params := 
                                map {
                                    ...
                                    "value": let $id := replace(@ref, ' ', '-') let $target:= id($id, doc("/db/apps/rqzh-data/organization/organization.xml")) let $letter := substring($id, 1, 1) => upper-case() return     if ($target) then ( <a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$target/orgName/string()}</a>, if ($target/@type) then( <span> ({$target/@type/string()})</span>) else () ) else (<a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$id}</a>)
                                }

                                                        let $content := 
                                model:template-orgName($config, ., $params)
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-orgName", "semantic", "organization", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(summary) return
                        epub:block($config, ., ("tei-summary", css:map-rend-to-class(.)), .)
                            ...
                            if (parent::origin and text()) then
                                ...
                                if (parent::origin) then
                                    ...
                                    if (@calendar and @from and @to and ancestor::body and not(ancestor::note)) then
                                        epub:alternate($config, ., ("tei-origDate4", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('origDate'), ec:colon(), ec:format-date(@from), ' – ', ec:format-date(@to), ' (', ec:translate(@calendar, 0, ()), ')'))
                                    else
                                        if (@calendar) then
                                            epub:alternate($config, ., ("tei-origDate5", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('origDate'), ec:colon(), ec:format-date(@when), ' (', ec:translate(@calendar, 0, ()), ')'))
                                        else
                                            if (@from and @to and ancestor::body and not(ancestor::note)) then
                                                epub:alternate($config, ., ("tei-origDate6", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('origDate'), ec:colon(), ec:format-date(@from), ' – ', ec:format-date(@to)))
                                            else
                                                epub:alternate($config, ., ("tei-origDate7", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('origDate'), ec:colon(), ec:format-date(@when)))
                    case element(p) return
                        if (parent::seg[@n] and parent::seg/*[1] is .) then
                            (: Put seg numbering before the first p but on the same line :)
                            html:paragraph($config, ., css:get-rendition(., ("tei-p1", css:map-rend-to-class(.))), ('[', parent::seg/@n, '] ', .))
                        else
                            if (parent::body/div and parent::div[@n]/*[1] is .) then
                                html:paragraph($config, ., css:get-rendition(., ("tei-p2", css:map-rend-to-class(.))), ('[', parent::div/@n, '] ', .))
                            else
                    ...
                    case element(measure) return
                        if (ancestor::measureGrp and (session:get-attribute('ssrq.lang'), 'de')[1]!='de') then
                            ...
                            epub:alternate($config, ., ("tei-measure1", "text-critical", css:map-rend-to-class(.)), ., ., let $measures := ancestor::measureGrp//measure  return (  ec:translate($measures[1]/@type, 0, 'uppercase'), ec:colon(),           for $measure in $measures     return     (' ', $measure/@quantity, ' ', ec:translate($measure/@unit, $measure/@quantity, 'lowercase'), ' ', ec:translate($measure/@commodity, 0, 'lowercase'), ' ', ec:translate($measure/@origin, 0, 'lowercase')) ))
                        else
                            if (ancestor::measureGrp and (session:get-attribute('ssrq.lang'), 'de')[1]='de') then
                                (: Display summarized measures for a measure group in a tooltip (German) :)
                                epub:alternate($config, ., ("tei-measure2", "text-critical", css:map-rend-to-class(.)), ., ., let $measures := ancestor::measureGrp//measure  return (  ec:translate($measures[1]/@type, 0, 'uppercase'), ec:colon(),           for $measure in $measures     return     (' ', $measure/@quantity, ' ', ec:translate($measure/@origin, 0, 'lowercase'), ' ', ec:translate($measure/@unit, $measure/@quantity, 'lowercase'), ' ', ec:translate($measure/@commodity, 0, 'lowercase')) ))
                            else
                                if ((session:get-attribute('ssrq.lang'), 'de')[1]='de') then
                                    ...
                                    epub:alternate($config, ., ("tei-measure3", "text-critical", css:map-rend-to-class(.)), ., ., (ec:translate(@type, 0, 'uppercase'), ec:colon(), @quantity, ' ', ec:translate(@origin, 0, 'lowercase'), ' ', ec:translate(@unit, @quantity, 'lowercase'), ' ', ec:translate(@commodity, 0, 'lowercase')))
                                else
                                    if ((session:get-attribute('ssrq.lang'), 'de')[1]!='de') then
                                        (: Display measurement with given parameters in tooltip and highlight text with color :)
                                        epub:alternate($config, ., ("tei-measure4", "text-critical", css:map-rend-to-class(.)), ., ., (ec:translate(@type, 0, 'uppercase'), ec:colon(), @quantity, ' ', ec:translate(@unit, @quantity, 'lowercase'), ' ', ec:translate(@commodity, 0, 'lowercase'), ' ', ec:translate(@origin, 0, 'lowercase')))
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(lb) return
                        html:omit($config, ., ("tei-lb", css:map-rend-to-class(.)), .)
                        ...
                        if ($parameters?header='context') then
                            let $params := 
                                map {
                                    "content": .,
                                    "ref": @xml:id,
                                    "value": ( <a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={@xml:id}" target="_blank">{./persName/string()}</a>, if (string-length(./note) > 0) then( <span> ({./note/string()})</span>) else () )
                                }

                                                        let $content := 
                                model:template-person($config, ., $params)
                            return
                                                        html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-person", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                                ...
                                if (@ref) then
                                    ...
                                    let $params := 
                                        map {
                                            ...
                                            "value": let $id := replace(@ref, ' ', '-') let $target:= id($id, doc("/db/apps/rqzh-data/person/person.xml")) let $letter := substring($id, 1, 1) => upper-case() return     if ($target) then ( <a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$target/persName/string()}</a>, if ($target/note) then( <span> ({$target/note/string()} )</span>) else () ) else (<a href="https://www.ssrq-sds-fds.ch/persons-db-edit/?query={$id}" target="_blank">{$id}</a>)
                                        }
                    ...
                    case element(placeName) return
                        if (@ref) then
                            ...
                            return
                                                        html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-placeName", "semantic", "place", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(num) return
                        ...
                        epub:alternate($config, ., ("tei-num", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('amount'), ec:colon(), @value))
                    ...
                    case element(ref) return
                        if (parent::bibl) then
                            html:link($config, ., ("tei-ref1", "ref-link", css:map-rend-to-class(.)), ., (), (), map {"text": ., "link": ec:ref-link(@target, util:collection-name(.))})
                        else
                            if (ancestor::div/@type='collection') then
                                (: Link in Klammerdokument :)
                                html:link($config, ., ("tei-ref2", css:map-rend-to-class(.)), (doc(util:collection-name(.) || '/' || . || '.xml')//sourceDesc/msDesc/head/text(), ' (', ec:format-id(.), ')'), (), (), map {"link": string() || '.xml'})
                            else
                                if (not(@target) and matches(., '^(?:SSRQ|SDS|FDS)_')) then
                                    (: Link auf ein anderes Dokument innerhalb Portal :)
                                    html:link($config, ., ("tei-ref3", css:map-rend-to-class(.)), ., (), (), map {"link": string() || ".xml"})
                                else
                                    if (not(@target)) then
                                        ...
                                        if (not(text())) then
                                            html:link($config, ., ("tei-ref5", css:map-rend-to-class(.)), @target, (), (), map {"link": @target})
                                        else
                                            html:link($config, ., ("tei-ref6", css:map-rend-to-class(.)), ., (), '_new', map {"link": @target})
                            ...
                            if (parent::subst) then
                                ...
                                if (parent::del) then
                                    ...
                                    if (@hand!='other hand') then
                                        (: Show footnote only :)
                                        epub:note($config, ., ("tei-add4", css:map-rend-to-class(.)), ., (), ())
                                    else
                    ...
                    case element(head) return
                        if (@resp and @n) then
                            ...
                            html:heading($config, ., ("tei-head1", css:map-rend-to-class(.)), ('[ ', @n, ' ', ., ' ]'), if (@type='title') then 1 else 2)
                        else
                            if (@n) then
                                (: Supplied heading: n. in brackets unless @resp :)
                                html:heading($config, ., ("tei-head2", css:map-rend-to-class(.)), ('[ ', @n, ' ] ', .), if (@type='title') then 1 else 2)
                            else
                                if (@resp) then
                                    (: Supplied heading: output in brackets if @resp :)
                                    html:heading($config, ., ("tei-head3", css:map-rend-to-class(.)), ., if (@type='title') then 1 else 2)
                                else
                                    if ($parameters?header='short') then
                                        html:inline($config, ., ("tei-head4", css:map-rend-to-class(.)), replace(string-join(.//text()[not(parent::ref)]), '^(.*?)[^\w]*$', '$1'))
                                    else
                                                ...
                                                if (parent::lg) then
                                                    ...
                                                    if (parent::list) then
                                                        ...
                                                        if (ancestor::app) then
                                                            (: Heading in app :)
                                                            html:inline($config, ., ("tei-head9", "head", css:map-rend-to-class(.)), .)
                                                        else
                                                            if (parent::div[@n][@type=('chapter', 'section')]/*[1] is .) then
                                                                (: Show number in front of heading if @n is set on parent :)
                                                                html:heading($config, ., ("tei-head10", css:map-rend-to-class(.)), ('[', parent::div/@n, '] ', .), 1)
                                                            else
                                                                if (parent::div[@n]/*[1] is .) then
                                                                    (: Show number in front of heading if @n is set on parent :)
                                                                    html:heading($config, ., ("tei-head11", css:map-rend-to-class(.)), ('[', parent::div/@n, '] ', .), if (@type=('chapter','section','title')) then 1 else 2)
                                                                else
                                                                    (: All other headings are same size as text :)
                                                                    html:heading($config, ., ("tei-head12", css:map-rend-to-class(.)), ., if (@type='title') then 1 else 2)
                    ...
                    case element(time) return
                        if (@period) then
                            ...
                            epub:alternate($config, ., ("tei-time1", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('duration'), ec:colon(), ec:translate(@period, 0, ())))
                        else
                            if (@when) then
                                (: Display time with given parameter in tooltip and highlight text with color :)
                                epub:alternate($config, ., ("tei-time2", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('time'), ec:colon(), format-time(@when, '[H]:[m]')))
                            else
                                if (@dur) then
                                    (: Display time in tooltip and highlight text with color :)
                                    epub:alternate($config, ., ("tei-time3", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('duration'), ec:colon(), ec:format-duration(@dur)))
                                else
                                    (: Display time in tooltip and highlight text with color :)
                                    epub:alternate($config, ., ("tei-time4", "text-critical", css:map-rend-to-class(.)), ., ., ec:label('time'))
                    ...
                    case element(date) return
                        if (@when and @type) then
                            epub:alternate($config, ., ("tei-date1", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('date'), ec:colon(), ec:format-date(@when), ' (', ec:translate(@type, 0, ()), ')'))
                        else
                            if (@when and @calendar) then
                                epub:alternate($config, ., ("tei-date2", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('date'), ec:colon(), ec:format-date(@when), ' (', ec:translate(@calendar, 0, ()), ')'))
                            else
                                if (@when) then
                                    epub:alternate($config, ., ("tei-date3", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('date'), ec:colon(), ec:format-date(@when)))
                                else
                                    if (@from and @to) then
                                        epub:alternate($config, ., ("tei-date4", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('date'), ec:colon(), ec:format-date(@from), ' – ', ec:format-date(@to)))
                                    else
                                        if (@period) then
                                            epub:alternate($config, ., ("tei-date5", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('duration'), ec:colon(), ec:translate(@period, 0, ())))
                                        else
                                            if (@dur[ends-with(., 'W')]) then
                                                (: Alternate (popover) for week duration labels in custom format e.g. 'R/P1W'. :)
                                                epub:alternate($config, ., ("tei-date6", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('repeated-duration'), ec:colon(),ec:format-week-duration(substring-after(@dur, 'R/'))))
                                            else
                                                if (@dur[starts-with(., 'R/P')]) then
                                                    epub:alternate($config, ., ("tei-date7", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('repeated-duration'), ec:colon(), ec:format-duration(substring-after(@dur, 'R/'))))
                                                else
                                                    if (@dur) then
                                                        epub:alternate($config, ., ("tei-date8", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('duration'), ec:colon(), ec:format-duration(@dur)))
                                                    else
                                                        if (@type) then
                                                            epub:alternate($config, ., ("tei-date9", "text-critical", css:map-rend-to-class(.)), ., ., (ec:label('date'), ' ', ec:translate(@type, 0, ())))
                                                        else
                                                            if (text()) then
                    ...
                    case element(corr) return
                        if (parent::choice) then
                            ...
                            epub:alternate($config, ., ("tei-corr2", css:map-rend-to-class(.)), ., ., ec:label('corr'))
                    ...
                    case element(sic) return
                        if (parent::choice and count(parent::*/*) gt 1) then
                            ...
                            epub:alternate($config, ., ("tei-sic2", "text-critical", css:map-rend-to-class(.)), ., ., ec:label('sic'))
                    case element(titleStmt) return
                        if ($parameters?header='short') then
                            (
                                epub:block($config, ., ("tei-titleStmt2", css:map-rend-to-class(.)), subsequence(title, 2)),
                                epub:block($config, ., ("tei-titleStmt3", css:map-rend-to-class(.)), author)
                            )

                        else
                            epub:block($config, ., ("tei-titleStmt4", css:map-rend-to-class(.)), .)
                    ...
                    case element(body) return
                        (
                            ...
                            epub:block($config, ., ("tei-body2", "body", css:map-rend-to-class(.)), .)
                        )
                    ...
                    case element(gap) return
                        if (@reason='irrelevant') then
                            epub:alternate($config, ., ("tei-gap1", "text-critical", css:map-rend-to-class(.)), ., text{'[...]'}, ec:label('irrelevant'))
                        else
