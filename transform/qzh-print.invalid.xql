...
module namespace model="http://www.tei-c.org/pm/models/qzh/fo";
...
import module namespace ext-fo="http://www.tei-c.org/tei-simple/xquery/ext-fo" at "xmldb:exist:///db/apps/qzh/modules/ext-fo.xql";
            ...
            return
                    ...
                    case element(choice) return
                        if (sic and corr) then
                            ext-fo:alternote($config, ., ("tei-choice1", "text-critical", css:map-rend-to-class(.)), sic, (), 'text-critical', corr)
                        else
                            if (abbr and expan) then
                                ext-fo:alternate($config, ., ("tei-choice2", "text-critical", css:map-rend-to-class(.)), ., abbr[1], expan[1])
                            else
                                if (orig and reg) then
                                    ext-fo:alternate($config, ., ("tei-choice3", "text-critical", css:map-rend-to-class(.)), ., orig[1], reg[1])
                                else
                                    $config?apply($config, ./node())
                    ...
                    case element(seg) return
                        fo:inline($config, ., ("tei-seg", css:map-rend-to-class(.)), .)
                    ...
                    case element(lb) return
                        if (ancestor::back//orig) then
                            ...
                            if (preceding-sibling::node()[1][self::pb]) then
                                fo:omit($config, ., ("tei-lb2", css:map-rend-to-class(.)), .)
                            else
                                if (. is ancestor::div/node()[1]) then
                                    fo:omit($config, ., ("tei-lb3", css:map-rend-to-class(.)), .)
                                else
                                    if (. is ancestor::p/node()[1]) then
                                        ...
                                        if (@break='no') then
                                            (
                                                fo:inline($config, ., ("tei-lb5", css:map-rend-to-class(.)), .),
                                                fo:break($config, ., ("tei-lb6", css:map-rend-to-class(.)), ., 'page', @n)
                                            )

                                        else
                                            fo:break($config, ., ("tei-lb7", css:map-rend-to-class(.)), ., 'line', @n)
                    ...
                    case element(head) return
                        if (@resp) then
                            fo:omit($config, ., ("tei-head1", css:map-rend-to-class(.)), .)
                        else
                            if (@type='title') then
                                fo:heading($config, ., ("tei-head2", css:map-rend-to-class(.)), ., 1)
                            else
                                if (@type='subtitle') then
                                    fo:heading($config, ., ("tei-head3", css:map-rend-to-class(.)), ., 2)
                                else
                                    if ($parameters?header='short') then
                                        fo:heading($config, ., ("tei-head5", css:map-rend-to-class(.)), replace(string-join(.//text() except .//ref//text()), '^(.*?)[^\w]*$', '$1'), ())
                                    else
                                        if (parent::figure) then
                                            ...
                                            if (parent::table) then
                                                ...
                                                if (parent::lg) then
                                                    ...
                                                    if (parent::list) then
                                                        fo:block($config, ., ("tei-head9", css:map-rend-to-class(.)), .)
                                                    else
                                                        if (ancestor::app) then
                                                            (: Heading in app (don't format) :)
                                                            fo:inline($config, ., ("tei-head10", css:map-rend-to-class(.)), .)
                                                        else
                                                            fo:heading($config, ., css:get-rendition(., ("tei-head4", css:map-rend-to-class(.))), ., 2)
                    ...
                    case element(orgName) return
                        if (@ref) then
                            ...
                            return
                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-orgName", "semantic", "organization", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(placeName) return
                        if (@ref) then
                            ...
                            return
                                                        fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-placeName", "semantic", "place", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
