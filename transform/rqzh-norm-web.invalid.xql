...
module namespace model="http://www.tei-c.org/pm/models/rqzh-norm/web";
            ...
            return
                    ...
                    case element(choice) return
                        if (sic/del and corr) then
                            ext-html:alternote($config, ., ("tei-choice4", "text-critical", css:map-rend-to-class(.)), corr, (), 'text-critical', (ec:label('corrected-from'), ec:colon(), normalize-space(string-join(sic//text())), ', ', ec:translate(sic/del/@rend, 0, 'uppercase'), ec:colon(), sic/del//text()), map {})
                        else
                            if (sic and corr) then
                                ext-html:alternote($config, ., ("tei-choice5", "text-critical", css:map-rend-to-class(.)), corr, (), 'text-critical', (ec:label('corrected-from'), ec:colon(), sic), map {})
                            else
                                if (abbr and expan) then
                                    html:alternate($config, ., ("tei-choice6", "text-critical", css:map-rend-to-class(.)), ., expan[1], (ec:label('abbr-nonexpanded'), ec:colon(), abbr[1]), map {})
                                else
                                    if (orig and reg) then
                                        html:alternate($config, ., ("tei-choice7", "text-critical", css:map-rend-to-class(.)), ., reg[1], orig[1], map {})
                                    else
                                        $config?apply($config, ./node())
                    ...
                    case element(seg) return
                        if (@n and not(p)) then
                            html:block($config, ., ("tei-seg1", css:map-rend-to-class(.)), (text{'['}, @n, text{'] '}, .))
                        else
                            if (. is (ancestor::quote[1]/seg[1])) then
                                ...
                                if (. is (ancestor::quote[1]/seg[last()])) then
                                    ...
                                    if (. is (ancestor::quote[1]/seg[last()-1])) then
                                        (: Add missing margin :)
                                        html:block($config, ., ("tei-seg4", css:map-rend-to-class(.)), .)
                                    else
                                        html:block($config, ., ("tei-seg5", css:map-rend-to-class(.)), .)
                                    ...
                                    if ($parameters?header='short') then
                                        ...
                                        if (parent::figure) then
                                            ...
                                            if (parent::table) then
                                                ...
                                                ext-html:caption($config, ., ("tei-head6", css:map-rend-to-class(.)), .)
                                            else
                                                if (parent::lg) then
