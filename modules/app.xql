...
module namespace app="http://existsolutions.com/qzh/app";
...
function app:download-pdf($node as node(), $model as map(*)) {
    ...
    return
        <a href="{request:get-context-path()}/apps/qzh-data/data/{$resource}">
        {
            $node/@*,
...
function app:download-xml($node as node(), $model as map(*)) {
    <a href="{request:get-context-path()}/apps/qzh-data/{$model?doc}">
    {
        $node/@*,
        templates:process($node/node(), $model)
    }
...
declare function app:meta($node as node(), $model as map(*)) {
    ...
    return
        map {
            ...
            "language": "de",
            "url": "https://qzh.sources-online.org/" || $model?doc,
            "site": $site
        }
};
                ...
                <div>
                    ...
                    <div class="mentions">{
                            for $col in $config:data-collections
                                ...
                                let $matches := collection($config:data-root || "/" || $col)//tei:TEI[((.//tei:persName/@ref | @scribe) = $key) or (.//tei:idno/text()) = (substring-before($key, fn:concat('_', substring-after(substring-after($key,'_'), '_'))))]
